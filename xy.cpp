...
class Point {
   public:
    ...
    Point(double a, double b)
        : x(a), y(b) { /* Do nothing except initialize the values */
    }
...
void doSomethingWithAFile(const char* filename)
{
    ...
    if (fh == nullptr) throw std::runtime_error("Could not open the file.");
...
// the underlying object of a std::shared_ptr without causing that object to
// stay allocated. Weak pointers are used to prevent circular referencing.
...
for (int i = 0; i < my_vector.size(); i++) {
    cout
        << my_vector[i]
        << endl;  // for accessing a vector's element we can use the operator []
}
...
// available inside the function body and how. It can be either:
//     1. a value : [x]
//     2. a reference : [&x]
...
// You can use "auto" and not worry about the type of the elements of the
...
// C++ uses Short-circuit evaluation for boolean expressions, i.e, the second
// argument is executed or evaluated only if the first argument does not suffice
